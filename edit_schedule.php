...
$selected_friends = explode(',', $sched['friends']);
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $result = editSchedule($id, $game_id, $game, $date, $time, $friends_selected, $reminder);
    if ($result === true) {
            ...
            <form method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="game_id" name="game_id" required>
                        ...
                        <?php foreach ($games as $game): ?>
                            <option value="<?php echo $game['game_id']; ?>" <?php echo $game['game_id'] == $sched['game_id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($game['titel']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="mb-3">
                    <label for="game" class="form-label">Game Name</label>
                    <input type="text" class="form-control" id="game" name="game" value="<?php echo htmlspecialchars($sched['game']); ?>" required>
                </div>
                <div class="mb-3">
                    <label for="date" class="form-label">Date</label>
                    <input type="date" class="form-control" id="date" name="date" value="<?php echo $sched['date']; ?>" required>
                </div>
                <div class="mb-3">
                    <label for="time" class="form-label">Time</label>
                    <input type="time" class="form-control" id="time" name="time" value="<?php echo $sched['time']; ?>" required>
                </div>
                <div class="mb-3">
                    <label>Friends to Share With</label>
                    <?php foreach ($friends as $friend): ?>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="friends[]" value="<?php echo $friend['friend_user_id']; ?>" <?php echo in_array($friend['friend_user_id'], $selected_friends) ? 'checked' : ''; ?>>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="reminder" name="reminder">
                        <option value="none" <?php echo $sched['reminder'] == 'none' ? 'selected' : ''; ?>>None</option>
                        <option value="1hour" <?php echo $sched['reminder'] == '1hour' ? 'selected' : ''; ?>>1 Hour Before</option>
                        <option value="1day" <?php echo $sched['reminder'] == '1day' ? 'selected' : ''; ?>>1 Day Before</option>
                    </select>
                </div>
